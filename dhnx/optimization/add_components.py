            ...
            if b["excess"]:
                ...
                nodes.append(
                    solph.components.Sink(
                        ...
                        inputs={
                            busd[l_bus]: solph.Flow(variable_costs=b["excess costs"])
                        },
                    )
                )
            ...
            if b["shortage"]:
                ...
                nodes.append(
                    solph.components.Source(
                        ...
                        outputs={
                            busd[l_bus]: solph.Flow(variable_costs=b["shortage costs"])
                        },
                    )
                )
...
def add_sources(on, it, c, labels, nodes, busd):
    ...
    for _, cs in it.iterrows():
        ...
        spec_attr = [
            x
            for x in list(on.thermal_network.components[labels["l_1"]].columns)
        ...
        nodes.append(
            solph.components.Source(
                ...
                outputs={
                    busd[
                        (labels["l_1"], cs["label_2"], "bus", labels["l_4"])
...
def add_demand(it, labels, series, nodes, busd):
    ...
    for _, de in it.iterrows():
        ...
        inflow_args = {
            "nominal_value": de["nominal_value"],
        ...
        nodes.append(
            solph.components.Sink(
                ...
                inputs={
                    busd[
                        (labels["l_1"], labels["l_2"], "bus", labels["l_4"])
...
def add_transformer(it, labels, nodes, busd):
    ...
    for _, t in it.iterrows():
        ...
        if t["type"] == "1-in_1-out":
            ...
            b_out_1 = busd[(labels["l_1"], t["out_1"], "bus", labels["l_4"])]
...
def add_heatpipes(it, labels, bidirectional, length, b_in, b_out, nodes):
    ...
    for _, t in it.iterrows():
        ...
        flow_bi_args = {"bidirectional": True, "min": -1} if bidirectional else {}
...
def add_heatpipes_exist(pipes, labels, gd, q, b_in, b_out, nodes):
    ...
    nodes.append(
        oh.HeatPipeline(
            label=oh.Label(labels["l_1"], labels["l_2"], labels["l_3"], labels["l_4"]),
            inputs={
                b_in: solph.Flow(
