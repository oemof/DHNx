...
class SimulationModelNumpy(SimulationModel):
    ...
    def __init__(
        ...
        self.inc_mat = nx.incidence_matrix(
            self.nx_graph, oriented=True
        ).todense()
        ...
        self.temp_env = thermal_network.sequences.environment.temp_env.iloc[
            :, 0
        ]
    ...
    def prepare_hydraulic_eqn(self):
        ...
        self.input_data.mass_flow = self._set_producers_mass_flow(
            self.input_data.mass_flow
        )
    ...
    def solve_hydraulic_eqn(self):
        ...
        pipes_dist_pressure_losses = (
            self._calculate_pipes_distributed_pressure_losses(lamb)
        )

        pipes_loc_pressure_losses = (
            self._calculate_pipes_localized_pressure_losses()
        )
        ...
        global_pressure_losses = self._calculate_global_pressure_losses(
            pipes_total_pressure_losses
        )
    ...
    def solve_thermal_eqn(self):
        ...
        temp_inlet = self._calc_temps(
            exponent_constant, self.input_data.temp_inlet, direction=1
        )

        temp_return_known = self._set_temp_return_input(temp_inlet)

        temp_return = self._calc_temps(
            exponent_constant, temp_return_known, direction=-1
        )

        pipes_heat_losses = self._calculate_pipes_heat_losses(
            temp_inlet
    ...
    def _concat_scalars(self, name):
        ...
        select_scalars = [
            scalar[name]
            .copy()
            .rename(index=lambda x, prefix=component: prefix + "-" + str(x))
    ...
    def _concat_sequences(self, name):
        ...
        select_sequences = [
            d[name]
            .copy()
            .rename(columns=lambda x, prefix=component: prefix + "-" + x)
    ...
    def _set_producers_mass_flow(m):
        ...
        """
        producers = [
            name for name in m.columns if re.search("producers", name)
        ]
    ...
    def _calculate_pipes_mass_flow(self):
        ...
        pipes_mass_flow = pd.DataFrame.from_dict(
            pipes_mass_flow, orient="index", columns=self.nx_graph.edges()
        )
    ...
    def _calculate_reynolds(self):
        ...
        diameter = self.thermal_network.components.pipes[
            ["from_node", "to_node", "diameter"]
        ]

        diameter = (
            1e-3 * diameter.set_index(["from_node", "to_node"])["diameter"]
        )

        reynolds = (
            4
            * abs_pipes_mass_flow.divide(diameter, axis="columns")
            / (np.pi * self.mu)
        )
    ...
    def _calculate_lambda(self, reynolds):
        ...
        factor_diameter = (
            1e-3
            * factor_diameter.set_index(["from_node", "to_node"])["diameter"]
        ) ** -0.14
    ...
    def _calculate_pipes_distributed_pressure_losses(self, lamb):
        ...
        length = self.thermal_network.components.pipes[
        ...
        pipes_pressure_losses = constant * pipes_mass_flow_2.multiply(
            length, axis="columns"
    ...
    def _calculate_pipes_localized_pressure_losses(self):
        ...
        def _assign_zeta_to_pipes(flow_type):
            ...
            zeta_pipes = pd.DataFrame(
                columns=flow_direction.columns, index=flow_direction.index
            )
            ...
            zeta_pipes_backward = zeta_pipes_backward.apply(
                func_backward, axis=1
            )
        ...
        def _calc_loc_pressure_loss_for_flow_type(flow_type):
            ...
            for t in self.thermal_network.timeindex:
                ...
                mass_flow_2_over_diameter_4.name = (
                    "mass_flow_2_over_diameter_4"
                )

                x = constant * zeta_pipes.loc[t, :].multiply(
                    mass_flow_2_over_diameter_4
                )
            ...
            pipes_localized_pressure_losses = pd.DataFrame.from_dict(
                pipes_localized_pressure_losses, orient="index"
            )

            pipes_localized_pressure_losses = (
                pipes_localized_pressure_losses.fillna(0)
            )
        ...
        pipes_localized_pressure_losses_inlet = (
            _calc_loc_pressure_loss_for_flow_type("inlet")
        )

        pipes_localized_pressure_losses_return = (
            _calc_loc_pressure_loss_for_flow_type("return")
        )
    ...
    def _calculate_global_pressure_losses(self, pipes_pressure_losses):
            ...
            for sink in sink_nodes:
                for source in source_nodes:
                    ...
                    path_weights[sink] = nx.dijkstra_path_length(
                        graph, source=source, target=sink, weight=weight
                    )
        ...
        def _change_graph_dir(graph, direction):
            ...
            dir_edges = [
                e if direction.loc[_get_tuple(e)] > 0 else _swap_tuple(e)
        ...
        def _calculate_paths_pressure_losses():
            ...
            for t in self.thermal_network.timeindex:
                ...
                flow_dir_graph = _change_graph_dir(
                    graph, flow_direction.loc[t, :]
                )
            ...
            paths_pressure_losses = pd.DataFrame.from_dict(
                paths_pressure_losses, orient="index"
            )
    ...
    def _calculate_exponent_constant(self):
        ...
        heat_transfer_coefficient = nx.adjacency_matrix(
            self.nx_graph, weight="heat_transfer_coefficient"
        ).todense()

        diameter = (
            1e-3
            * nx.adjacency_matrix(self.nx_graph, weight="diameter").todense()
        )
        ...
        exponent_constant = (
            -np.pi
            * np.multiply(
                heat_transfer_coefficient, np.multiply(diameter, length)
            )
            / self.c
        )
        ...
        for t in self.thermal_network.timeindex:
            ...
            if direction == 1:
                ...
                normalisation = np.array(
                    nx.adjacency_matrix(self.nx_graph).sum(0)
                ).flatten()

                normalisation = np.divide(
                    np.array([1]), normalisation, where=normalisation != 0
                )

            elif direction == -1:
                normalisation = np.array(
                    nx.adjacency_matrix(self.nx_graph).sum(1)
                ).flatten()
            ...
            x, _, _, _ = np.linalg.lstsq(matrix, vector, rcond=None)

            temps.update({t: x + self.temp_env.loc[t]})

        temp_df = pd.DataFrame.from_dict(
            temps, orient="index", columns=self.nx_graph.nodes()
        )
    ...
    def _set_temp_return_input(self, temp_inlet):
        ...
        temp_return.loc[:, temp_drop.columns] = (
            temp_inlet.loc[:, temp_drop.columns] - temp_drop
        )
    ...
    def _calculate_pipes_heat_losses(self, temp_node):
        ...
        for i, row in temp_node.iterrows():
            ...
            temp_difference = np.abs(
                np.array(np.dot(row, self.inc_mat)).flatten()
            )

            pipes_heat_losses[i] = self.c * mass_flow.multiply(
                temp_difference, axis=0
            )

        pipes_heat_losses = pd.DataFrame.from_dict(
            pipes_heat_losses, orient="index"
        )
