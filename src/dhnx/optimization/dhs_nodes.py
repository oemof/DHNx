...
def add_nodes_dhs(opti_network, gd, nodes, busd):
    ...
    *OemofInvestOptimisationModel*, the oemof-solph components for the district
    ...
    for n, _ in opti_network.thermal_network.components["forks"].iterrows():
        ...
        l_bus = oh.Label(
            d_labels["l_1"], d_labels["l_2"], d_labels["l_3"], d_labels["l_4"]
        )
...
def add_nodes_houses(opti_network, nodes, busd, label_1):
    ...
    *Sources* and *Sinks* can be added.
    ...
    gen_data = opti_network.invest_options[
        label_1
    ]  # genic data for all houses
    ...
    for r, c in opti_network.thermal_network.components[label_1].iterrows():
        ...
        nodes, busd = ac.add_buses(gen_data["bus"], d_labels, nodes, busd)

        for key, item in gen_data.items():

            if key == "source":
                nodes = ac.add_sources(
                    opti_network, item, c, d_labels, nodes, busd
                )
