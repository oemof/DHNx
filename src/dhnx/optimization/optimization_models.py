...
class OemofInvestOptimizationModel(InvestOptimizationModel):
    ...
    def check_existing(self):
        ...
        if "hp_type" not in list(
            self.thermal_network.components["pipes"].columns
        ):
        ...
        hp_list = list(
            {x for x in edges["hp_type"].tolist() if isinstance(x, str)}
        )
        ...
        def catch_up_results():
            ...
            def check_invest_label(hp_type, edge_id):
                ...
                if isinstance(hp_type, str):
                    raise ValueError(
                        "Pipe id {} already has an investment > 0!".format(
                            edge_id
                        )
                    )
...
def setup_optimise_investment(
    ...
    model = OemofInvestOptimizationModel(
        thermal_network, settings, invest_options
    )
...
def solve_optimisation_investment(model):
    ...
    if model.settings["dump_path"] is not None:
        my_es = model.es
        my_es.dump(
            dpath=model.settings["dump_path"],
            filename=model.settings["dump_name"],
        )
        logger.info(
            'oemof Energysystem stored in "{}"'.format(
                model.settings["dump_path"]
            )
        )
