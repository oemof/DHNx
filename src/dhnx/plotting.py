...
class InteractiveMap:
    ...
    def collect_node_data(thermal_network):
        node_data = {
            list_name: thermal_network.components[list_name].copy()
    ...
    def _add_colors(self):
        ...
        self.node_data = self.node_data.assign(
            node_color=self.node_data["component_type"]
    ...
    def draw(self):
        # create map
        m = fol.Map(location=[self.lat.mean(), self.lon.mean()], zoom_start=14)
        ...
        for i in range(0, len(self.edge_data)):
            ...
            fol.PolyLine(
                locations=[
                    [
                        self.lat[self.edge_data["from_node"][i]],
...
class StaticMap:
    ...
    def __init__(
        ...
        self.positions = {
            node_id: np.array([data["lon"], data["lat"]])
    ...
    def draw(
        ...
        if background_map:
            if not cartopy_installed:
                logger.warning(
                    "To draw background map, cartopy must be installed."
                )
                background_map = False
        ...
        if background_map:
            ...
            zoom_level = 15
            fig, ax = plt.subplots(
                figsize=self.figsize, subplot_kw={"projection": imagery.crs}
            )
            ax.set_extent(self.extent, crs=ccrs.Geodetic())
            ax.add_image(
                imagery, zoom_level, alpha=1, interpolation="bilinear"
            )
