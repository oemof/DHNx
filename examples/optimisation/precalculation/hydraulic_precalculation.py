...
from dhnx.optimization.precalc_hydraulic import (
    v_max_bisection,
    calc_mass_flow,
    calc_power,
    v_max_secant,
...
df["v_max [m/s]"] = df.apply(
    lambda row: v_max_bisection(
        d_i=row["Inner diameter [m]"],
        T_average=row["Temperature level [Celsius]"],
        k=row["Roughness [mm]"],
        p_max=maximum_pressure_drop,
    ),
    axis=1,
)

# alternative with secant method
df["v_max (secant) [m/s]"] = df.apply(
    lambda row: v_max_secant(
        d_i=row["Inner diameter [m]"],
        T_average=row["Temperature level [Celsius]"],
        k=row["Roughness [mm]"],
        p_max=maximum_pressure_drop,
...
ax.plot([x_min, x_max], [y_min, y_max], ls=":", color="r", marker="x")
