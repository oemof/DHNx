...
class lin_consum_open(subsystem):
    ...
    def create_comps(self):
        ...
        for i in range(self.num_consumer):
            ...
            self.comps['consumer_' + j] = heat_exchanger_simple(
                self.label + '_consumer_' + j
            )
    ...
    def create_conns(self):
        ...
        for i in range(self.num_consumer):
            j = str(i)
            if i > 0:
                self.conns['fesp_' + j] = connection(
                    self.comps['feed_' + str(i - 1)],
                    'out1',
                    self.comps['splitter_' + j],
                    'in1',
                )
                self.conns['mere_' + j] = connection(
                    self.comps['merge_' + j],
                    'out1',
                    self.comps['return_' + str(i - 1)],
                    'in1',
                )

            self.conns['spco_' + j] = connection(
                self.comps['splitter_' + j], 'out1', self.comps['consumer_' + j], 'in1'
            )
            self.conns['cova_' + j] = connection(
                self.comps['consumer_' + j], 'out1', self.comps['valve_' + j], 'in1'
            )
            self.conns['vame_' + j] = connection(
                self.comps['valve_' + j], 'out1', self.comps['merge_' + j], 'in2'
            )

            if i < self.num_consumer - 1:
                self.conns['spfe_' + j] = connection(
                    self.comps['splitter_' + j], 'out2', self.comps['feed_' + j], 'in1'
                )
                self.conns['reme_' + j] = connection(
                    self.comps['return_' + j], 'out1', self.comps['merge_' + j], 'in1'
                )
...
class lin_consum_closed(subsystem):
    ...
    def create_conns(self):
        ...
        self.conns['spco_' + str(i + 1)] = connection(
            self.comps['feed_' + j], 'out1', self.comps['consumer_' + str(i + 1)], 'in1'
        )
        self.conns['cova_' + str(i + 1)] = connection(
            self.comps['consumer_' + str(i + 1)],
            'out1',
            self.comps['return_' + j],
            'in1',
        )
...
class fork(subsystem):
    ...
    def create_conns(self):
        ...
        for i in range(self.num_branch):
            ...
            k = str(i + 1)
            self.conns['vame_' + j] = connection(
                self.comps['valve_' + j], 'out1', self.comps['merge'], 'in' + k
            )
